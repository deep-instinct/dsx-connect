#!/usr/bin/env python3
"""Run OneDrive connector locally without Docker/K8s."""
...
DEFAULT_STATE_DIR = Path.home() / ".dsx-connect-local" / "onedrive-connector"
DEFAULT_PORT = 8660

app = typer.Typer(help="OneDrive connector local runtime manager")
...
def _is_repo_root(path: Path) -> bool:
    return (path / "connectors" / "onedrive" / "start.py").exists()
...
def _default_env_template(*, state_dir: Path, port: int) -> str:
    _ = state_dir
    return f"""# OneDrive connector local runtime env (generated)
...
def _service_spec(state_dir: Path, *, port: int, host: str, workers: int, reload: bool) -> ServiceSpec:
    ...
    cmd = [
        python,
        str(repo / "connectors" / "onedrive" / "start.py"),
        "--host",
        host,
        "--port",
        str(port),
    ...
    return ServiceSpec(
        ...
        command=cmd,
        logfile=paths["logs"] / "onedrive-connector.log",
        pidfile=paths["run"] / "onedrive-connector.pid",
        cwd=repo,
    )
