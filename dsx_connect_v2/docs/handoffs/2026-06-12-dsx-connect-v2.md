...
`dsx_connect_v2` is the standalone next-generation DSX-Connect application boundary. It is intentionally separate from legacy `dsx_connect` and owns the new control-plane-first architecture, job orchestration model, worker contracts, result sink pipeline, local runtime, and operator UI routes.
...
```text
dsx_connect_v2/README.md
dsx_connect_v2/dsx_connect_v2/api/routes/ui.py
dsx_connect_v2/dsx_connect_v2/ui/operator_console.html
dsx_connect_v2/tests/test_ui_routes.py
```
...
```text
dsx_connect_v2/docs/
dsx_connect_v2/docs/index.md
dsx_connect_v2/docs/architecture.md
dsx_connect_v2/docs/api-boundaries.md
dsx_connect_v2/docs/runtime.md
dsx_connect_v2/docs/handoffs/2026-06-12-dsx-connect-v2.md
```
...
Updated the package-level README to point to package-local docs under `dsx_connect_v2/docs/` and reinforce the current API family boundaries, runtime commands, worker model, reader strategy model, and local preview guidance.
...
- `handoffs/2026-06-12-dsx-connect-v2.md`: this resume-state document

## Design Decisions That Must Not Be Reversed

1. `dsx_connect_v2` must remain isolated from legacy `dsx_connect`.
...
```bash
./.venv/bin/python -m pytest dsx_connect_v2/tests/test_ui_routes.py
```
...
./.venv/bin/python -m uvicorn dsx_connect_v2.app:app --host 127.0.0.1 --port 8093
```
...
- Decide whether the new package-local docs should be added to root `mkdocs.yml` or linked only from `dsx_connect_v2/README.md`.
...
2. Run a broader NG test pass and fix any regressions caused by the UI/API route additions.
   ...
   ```bash
   ./.venv/bin/python -m pytest dsx_connect_v2/tests
   ```
