...
usage() {
  cat <<'EOF'
Usage: scripts/dsx-connect-v2/deploy-k3s.sh --tag TAG [--registry REGISTRY] [--release NAME] [--namespace NS] [--values FILE] [--wait|--no-wait]

Deploys DSX-Connect v2 to the current Kubernetes context using Helm.

Examples:
  scripts/dsx-connect-v2/deploy-k3s.sh --tag dev --release dsx-connect --namespace dsx-connect
  scripts/dsx-connect-v2/deploy-k3s.sh --tag 2.0.0 --values dsx_connect_v2/deploy/helm/values-local.yaml --no-wait
EOF
}
