...
from dsx_connect_v2.jobs.contracts import ResultSinkEmitRequested
from dsx_connect_v2.workers import delivery_worker
...
def test_deliver_gateway_s3_posts_cached_artifact_to_connector(monkeypatch, tmp_path) -> None:
    artifact = tmp_path / "payload.txt"
    artifact.write_text("hello", encoding="utf-8")
    calls = []
    ...
    class _Response:
        ...
        def json(self):
            return {"status": "success", "uri": "s3://bucket-name/inbox/payload.txt"}
    ...
    monkeypatch.setattr(
        ...
        "_delivery_proxy_config",
        lambda _service, _request: delivery_worker.ConnectorProxyRuntimeConfig(
            endpoint_url="http://s3-connector/aws-s3-connector/write_file",
            timeout_seconds=10,
        ),
    )
    monkeypatch.setattr(delivery_worker.httpx, "AsyncClient", _Client)
    ...
    result = asyncio.run(
        delivery_worker.deliver_gateway_object_storage(
            _Service(),
            request,
            request.delivery_target.delivery_target,
        )
    ...
    assert result.outcome == "delivered"
    assert result.destination == "s3://bucket-name/inbox/payload.txt"
    assert result.details["worker"] == "s3_gateway_delivery"
    assert calls == [
        {
            "url": "http://s3-connector/aws-s3-connector/write_file",
            "data": {
                "bucket": "bucket-name",
                "key": "inbox/payload.txt",
                "destination": "s3://bucket-name/inbox/payload.txt",
            },
            "name": "payload.txt",
            "content": b"hello",
            "content_type": "application/octet-stream",
