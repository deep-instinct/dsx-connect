...
from dsx_connect_v2.jobs.bus import InMemoryJobBus, JobBus
from dsx_connect_v2.jobs.models import BatchJobSubmitRequest, JobSubmitRequest, OutboxFlushResult
from dsx_connect_v2.jobs.repository import InMemoryJobRepository
from dsx_connect_v2.jobs.service import JobService
from dsx_connect_v2.workers.relay_worker import PostgresOutboxWakeListener, build_outbox_wake_listener, relay_forever, relay_once
...
def test_build_outbox_wake_listener_uses_postgres_backend(monkeypatch) -> None:
    monkeypatch.setattr("dsx_connect_v2.workers.relay_worker.settings.postgres.url", "postgresql://example/db")
...
def test_relay_forever_logs_postgres_wakeup_result(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.workers.relay_worker.dsx_logging.isEnabledFor", lambda level: True)
    monkeypatch.setattr("dsx_connect_v2.workers.relay_worker.dsx_logging.log", lambda level, message: logged.append(json.loads(message)))
...
def test_relay_forever_suppresses_zero_work_info_logs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.workers.relay_worker.dsx_logging.isEnabledFor", fake_is_enabled)
    monkeypatch.setattr("dsx_connect_v2.workers.relay_worker.dsx_logging.log", lambda level, message: info_logs.append(json.loads(message)))
