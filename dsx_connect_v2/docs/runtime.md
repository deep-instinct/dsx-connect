...
```bash
python -m uvicorn dsx_connect_v2.app:app --host 127.0.0.1 --port 8091
```
...
This creates sample integrations, protected scopes, policies, and scan results for local preview. The endpoint is available only when `DSX_CONNECT_V2__ENVIRONMENT` is `dev`, `local`, or `test`.
