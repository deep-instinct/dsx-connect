...
These example configs show the recommended collector pattern for `dsx_connect_v2` result events:
...
```bash
vector --config dsx_connect_v2/examples/vector/vector-console.yaml
```

Environment variables commonly used by these configs:

```bash
export DSX_RESULTS_PATH=/tmp/dsx-connect-v2-results.jsonl
export VECTOR_DATA_DIR=/tmp/vector-dsx-connect-v2
