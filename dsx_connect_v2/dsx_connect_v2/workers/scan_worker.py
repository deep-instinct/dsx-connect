...
from dsx_connect_v2.config import settings
from dsx_connect_v2.control_plane.config_models import parse_integration_runtime_config
from dsx_connect_v2.jobs.contracts import MessageEnvelope, ScanItemRequested
from dsx_connect_v2.jobs.models import ScanResult, ScanStageUpdateRequest, StageUpdateRequest
from dsx_connect_v2.ops_logging import log_event, ops_logging
from dsx_connect_v2.readers.base import Reader, TerminalScanError
from dsx_connect_v2.readers.local_path import LocalPathReader
from dsx_connect_v2.readers.resolver import build_scan_reader
from dsx_connect_v2.jobs.service import JobService
from dsx_connect_v2.workers.consumer import consume_queue
from dsx_connect_v2.workers.runtime import build_job_service
...
_SCAN_BATCH_ITEM_LOGGING = _env_bool("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ITEM_LOGGING", True)
...
async def execute_scan_via_dsxa(request: ScanItemRequested, reader: Reader) -> ScanResult:
    ...
    if not base_url:
        raise TerminalScanError("scanner_base_url_required", "DSX_CONNECT_V2_SCANNER__BASE_URL is required for dsxa scan mode")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_BATCH_SIZE", "1")),
        help="Number of scan-only completions to buffer before bulk persistence.",
    )
    parser.add_argument(
        "--scan-only-completion-flush-interval-seconds",
        type=float,
        default=float(os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_FLUSH_INTERVAL_SECONDS", "1.0")),
        help="Maximum age for buffered scan-only completions before a bulk flush.",
    )
    parser.add_argument(
        "--scan-only-runtime-leases",
        action=argparse.BooleanOptionalAction,
        default=os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_ONLY_RUNTIME_LEASES", "true").lower() not in {"0", "false", "no", "off"},
        help="Record runtime scan leases for coarse scan-only batch work.",
    )
    parser.add_argument(
        "--scanner-client-scope",
        choices=["shared", "per-task"],
        default=os.environ.get("DSX_CONNECT_V2_LOCAL__SCANNER_CLIENT_SCOPE", "shared"),
        help="DSXA client lifetime used by the scan worker.",
    )
    parser.add_argument(
        "--service-io-threaded",
        action=argparse.BooleanOptionalAction,
        default=os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_WORKER_SERVICE_IO_THREADED", "false").lower()
        in {"1", "true", "yes", "on"},
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_SIZE", "1")),
        help="Collect this many coarse scan-only messages and scan them as one async gather batch.",
    )
    parser.add_argument(
        "--scan-batch-window-wait-seconds",
        type=float,
        default=float(os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_WAIT_SECONDS", "0.05")),
        help="Maximum wait for a partial scan-only batch window before scanning it.",
    )
    parser.add_argument(
        "--scan-batch-concurrency",
        type=int,
        default=int(os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_CONCURRENCY", "0")),
    ...
    parser.add_argument(
        ...
        default=os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ACK_MODE", "completed"),
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=os.environ.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_TRUST_ITEMS", "true").lower()
        in {"1", "true", "yes", "on"},
