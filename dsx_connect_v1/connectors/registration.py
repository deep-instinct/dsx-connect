...
from dsx_connect_v1.messaging.bus import Bus, SyncBus
...
def _effective_ttl(default_ttl: int | None = None) -> int:
    ...
    """
    try:
        from dsx_connect_v1.config import get_config, AppEnv  # local import to avoid circulars at import time
        cfg = get_config()
...
async def register_or_refresh_connector(
    ...
    if r is None:
        try:
            from dsx_connect_v1.config import get_config  # local import to avoid cycles
            cfg = get_config()
            dsx_logging.warning(
