...
cp dsx_connect_v1/.dev.env.example dsx_connect_v1/.dev.env   # edit as needed
```

`dsx_connect_v1/.dev.env` is loaded automatically by the helper scripts. Populate it with overrides such as:
...
The `dsx_connect_v1/*-start.py` scripts wrap the uvicorn / celery invocations and auto-load `.dev.env`. You can still override env vars inline when experimenting.

```bash
# API service (listens on 8586 by default)
LOG_LEVEL=debug \
DSXCONNECT_SCANNER__SCAN_BINARY_URL=http://localhost:5000/scan/binary/v2 \
python dsx_connect_v1/dsx-connect-api-start.py

# Celery workers (scan request + verdict + results + notification + dianna)
python dsx_connect_v1/dsx-connect-workers-start.py
```

Each worker class can also be launched individually via:

```bash
python dsx_connect_v1/dsx-connect-workers-start.py scan-request
python dsx_connect_v1/dsx-connect-workers-start.py verdict
python dsx_connect_v1/dsx-connect-workers-start.py results
python dsx_connect_v1/dsx-connect-workers-start.py notification
python dsx_connect_v1/dsx-connect-workers-start.py dianna
```
...
See `dsx_connect_v1/config.py` for the full BaseSettings definition.
...
- If you need alternate `.dev.env` variants, point the loader at a different file via `DSX_DEV_ENV_FILE=/path/to/custom.env python dsx_connect_v1/dsx-connect-api-start.py`.
...
Local runtime and demo-app documentation moved to:

- `dsx_connect_v1/local/README.md`
