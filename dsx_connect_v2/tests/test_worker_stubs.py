...
from dsx_connect_v2.control_plane.models import ConnectorInstanceRegister, IntegrationCreate, ProtectedScopeCreate
from dsx_connect_v2.control_plane.repository import InMemoryControlPlaneRepository
from dsx_connect_v2.control_plane.service import ControlPlaneService
from dsx_connect_v2.jobs.bus import InMemoryJobBus
from dsx_connect_v2.jobs.contracts import MessageEnvelope, PolicyEvaluationRequested, ResultSinkEmitRequested, ScanItemRequested
from dsx_connect_v2.jobs.models import (
...
from dsx_connect_v2.jobs.repository import InMemoryJobRepository
from dsx_connect_v2.jobs.service import JobService
from dsx_connect_v2.readers.proxy import ConnectorProxyReader, build_connector_proxy_reader, local_stub_connector_read, resolve_connector_proxy_runtime_config
from dsx_connect_v2.readers.gcs_native import GCSNativeReader
from dsx_connect_v2.readers.local_path import LocalPathReader
from dsx_connect_v2.readers.resolver import build_scan_reader, resolve_reader_strategy
from dsx_connect_v2.workers.delivery_worker import process_result_sink_message
from dsx_connect_v2.workers.dianna_worker import process_dianna_message
from dsx_connect_v2.workers.policy_worker import process_policy_message
from dsx_connect_v2.workers.policy_engine import stub_policy_engine
from dsx_connect_v2.workers.connector_actions import build_legacy_connector_action_payload
from dsx_connect_v2.workers.connector_actions import normalize_connector_remediation_response
from dsx_connect_v2.workers.remediation_worker import build_remediation_executor, process_remediation_message
from dsx_connect_v2.workers import scan_worker as scan_worker_module
from dsx_connect_v2.workers.scan_worker import (
...
def test_stub_remediation_executor_reports_tag_only() -> None:
    from dsx_connect_v2.jobs.contracts import RemediationRequested
    from dsx_connect_v2.workers.remediation_worker import stub_remediation_executor
...
def test_build_remediation_executor_falls_back_to_stub_when_connector_action_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.workers.remediation_worker.execute_connector_item_action", fail_connector_action)
...
def test_execute_scan_via_dsxa_enriches_scanner_metadata(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.base_url", "http://scanner.local")
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.dsxa_auth_token", "token")
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.timeout_seconds", 30.0)
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.verify_tls", True)
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.protected_entity", 1)
    ...
    monkeypatch.setattr(
        "dsx_connect_v2.workers.scan_worker._import_dsxa_client",
...
def test_execute_scan_via_dsxa_can_use_per_task_client_scope(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.verify_tls", True)
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.protected_entity", 1)
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker._SCANNER_CLIENT_SCOPE", "per-task")
...
def test_execute_scan_via_dsxa_does_not_default_protected_entity(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.timeout_seconds", 30.0)
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.verify_tls", True)
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.protected_entity", None)
    captured: dict[str, object] = {}
...
def test_resolve_scan_protected_entity_prefers_scope_override(monkeypatch) -> None:
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.protected_entity", 11)
    request = SimpleNamespace(
        scan_options={
            "integrationConfig": {"scanner": {"protected_entity": 77}},
...
def test_execute_scan_via_dsxa_can_scan_by_path_without_streaming_bytes(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.base_url", "http://scanner.local")
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.transport", "by_path")
    captured: dict[str, object] = {}
...
def test_execute_scan_via_dsxa_skips_oversize_by_size_hint(monkeypatch) -> None:
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.base_url", "http://scanner.local")
    monkeypatch.setattr("dsx_connect_v2.workers.scan_worker.settings.scanner.max_file_size_bytes", 100)
