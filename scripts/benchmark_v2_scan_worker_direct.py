...
for candidate in (REPO_ROOT, REPO_ROOT / "dsx_connect_v2"):
    if str(candidate) not in sys.path:
        sys.path.insert(0, str(candidate))


DEFAULT_ENV_FILE = Path.home() / ".dsx-connect-local" / "dsx-connect-v2" / ".env.local"
...
load_env_file(Path(os.environ.get("DSX_CONNECT_V2_DIRECT_ENV_FILE", str(DEFAULT_ENV_FILE))).expanduser())
...
from dsx_connect_v2.workers.scan_worker import execute_scan_via_dsxa
