...
DEFAULT_STATE_DIR = Path.home() / ".dsx-connect-local" / "dsx-connect-v2"
LOCAL_POSTGRES_RELAY_POLL_INTERVAL_SECONDS = 0.25
DSXA_DEFAULT_IMAGE = "dsxconnect/dpa-rocky9:4.2.0.2176"
...
DSX_CONNECT_V2_RABBITMQ__URL=amqp://dsx:dsx@127.0.0.1:5672/%2F
DSX_CONNECT_V2_POSTGRES__URL=postgresql://dsx:dsx@127.0.0.1:5432/dsx_connect_v2
DSX_CONNECT_V2_POSTGRES__AUTO_APPLY_SCHEMA=true
DSX_CONNECT_V2_LOCAL__SCAN_PREFETCH_COUNT=1000
DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT=1
DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_BATCH_SIZE=1
DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_FLUSH_INTERVAL_SECONDS=1.0
DSX_CONNECT_V2_LOCAL__SCAN_ONLY_RUNTIME_LEASES=false
DSX_CONNECT_V2_LOCAL__SCANNER_CLIENT_SCOPE=shared
# Optional when using --with-dsxa-docker.
# DSX_CONNECT_V2_LOCAL__DSXA_IMAGE=dsxconnect/dpa-rocky9:4.2.0.2176
# DSX_CONNECT_V2_LOCAL__DSXA_AUTH_TOKEN=
...
# FLAVOR=rest,config
# NO_SSL=true
DSX_CONNECT_V2_LOCAL__SCAN_WORKER_SERVICE_IO_THREADED=false
DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_SIZE=100
DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_WAIT_SECONDS=0.5
DSX_CONNECT_V2_LOCAL__SCAN_BATCH_CONCURRENCY=6
DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ACK_MODE=scanned
DSX_CONNECT_V2_LOCAL__SCAN_BATCH_TRUST_ITEMS=true
DSX_CONNECT_V2_LOCAL__POLICY_PREFETCH_COUNT=100
DSX_CONNECT_V2_LOCAL__RESULT_SINK_PREFETCH_COUNT=1
DSX_CONNECT_V2_RESULT_SINK__BACKEND=stdout
# DSX_CONNECT_V2_RESULT_SINK__BACKEND=json_lines
# DSX_CONNECT_V2_RESULT_SINK__PATH=/tmp/dsx-connect-v2-results.jsonl
DSX_CONNECT_V2__API_PREFIX=/api/v1
"""
...
def _service_specs(state_dir: Path, *, extra_env: dict[str, str] | None = None) -> list[ServiceSpec]:
    ...
    if "DSX_CONNECT_V2_POSTGRES__AUTO_APPLY_SCHEMA" in worker_env:
        worker_env["DSX_CONNECT_V2_POSTGRES__AUTO_APPLY_SCHEMA"] = "false"
    scan_worker_count = max(1, int(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT", "1")))
    scan_worker_specs = []
        ...
        scan_worker_specs.append(
            ServiceSpec(
                ...
                command=[
                    python,
                    "-m",
                    "dsx_connect_v2.workers.scan_worker",
                    "--prefetch-count",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_PREFETCH_COUNT", "1000")),
                    "--scan-only-completion-batch-size",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_BATCH_SIZE", "1")),
                    "--scan-only-completion-flush-interval-seconds",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_FLUSH_INTERVAL_SECONDS", "1.0")),
                    (
                        "--scan-only-runtime-leases"
                        if _env_bool(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_ONLY_RUNTIME_LEASES"), default=False)
                        else "--no-scan-only-runtime-leases"
                    ),
                    "--scanner-client-scope",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCANNER_CLIENT_SCOPE", "shared")),
                    (
                        "--service-io-threaded"
                        if _env_bool(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_WORKER_SERVICE_IO_THREADED"), default=False)
                        else "--no-service-io-threaded"
                    ),
                    "--scan-batch-window-size",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_SIZE", "100")),
                    "--scan-batch-window-wait-seconds",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_WAIT_SECONDS", "0.5")),
                    "--scan-batch-concurrency",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_CONCURRENCY", "6")),
                    "--scan-batch-ack-mode",
                    str(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ACK_MODE", "scanned")),
                    (
                        "--scan-batch-trust-items"
                        if _env_bool(base_child_env.get("DSX_CONNECT_V2_LOCAL__SCAN_BATCH_TRUST_ITEMS"), default=True)
                        else "--no-scan-batch-trust-items"
                    ),
                ],
                logfile=paths["logs"] / f"{scan_worker_name}.log",
                cwd=repo / "dsx_connect_v2",
                env=worker_env,
            )
        )
    return [
        ServiceSpec(
            name="api",
            command=[python, "-m", "uvicorn", "dsx_connect_v2.app:app", "--host", "127.0.0.1", "--port", "8091"],
        ...
        ServiceSpec(
            name="relay",
            command=[python, "-m", "dsx_connect_v2.workers.relay_worker"],
        ...
        *scan_worker_specs,
        ServiceSpec(
            ...
            command=[
                python,
                "-m",
                "dsx_connect_v2.workers.policy_worker",
                "--prefetch-count",
                str(base_child_env.get("DSX_CONNECT_V2_LOCAL__POLICY_PREFETCH_COUNT", "100")),
            ],
        ...
        ServiceSpec(
            ...
            command=[python, "-m", "dsx_connect_v2.workers.remediation_worker"],
        ...
        ServiceSpec(
            ...
            command=[
                python,
                "-m",
                "dsx_connect_v2.workers.result_sink_worker",
                "--prefetch-count",
                str(base_child_env.get("DSX_CONNECT_V2_LOCAL__RESULT_SINK_PREFETCH_COUNT", "1")),
            ],
        ...
        ServiceSpec(
            name="dianna-worker",
            command=[python, "-m", "dsx_connect_v2.workers.dianna_worker"],
...
def _ensure_dsxa_container(
    ...
    *,
    ...
    image = image.strip()
    if not image:
        raise RuntimeError("dsxa_image_required:set --dsxa-image, DSX_CONNECT_V2_LOCAL__DSXA_IMAGE, or DSXA_IMAGE")
    state = _rabbitmq_container_state(container_name)
    if state == "running":
...
def _dsxa_logs_show_ready(logs: str) -> bool:
    normalized = logs.lower()
    ready_markers = (
        "registration succeeded",
        "classifier initialized",
    )
    return any(marker in normalized for marker in ready_markers)


def _wait_for_dsxa_ready(container_name: str, host: str, port: int, *, timeout_seconds: float = 180.0) -> None:
    deadline = time.time() + timeout_seconds
    last_error: Exception | None = None
    while time.time() < deadline:
        state = _rabbitmq_container_state(container_name)
        logs = _docker_logs(container_name, tail=160)
        if _dsxa_logs_show_fatal_startup_failure(logs):
            raise RuntimeError(f"dsxa_not_ready:{container_name}:state={state or 'missing'}:fatal_startup_failure:logs={logs}")
...
def _wait_for_postgres_ready(container_name: str, *, timeout_seconds: float = 30.0) -> None:
    ...
    last_output = ""
    while time.time() < deadline:
        result = _docker_exec(container_name, ["pg_isready", "-U", "dsx", "-d", "dsx_connect_v2"])
        if result.returncode == 0:
            return
...
def _runtime_env_overrides(ctx: typer.Context) -> dict[str, str]:
    ...
    if ctx.obj.get("with_postgres_docker"):
        overrides["DSX_CONNECT_V2__CONTROL_PLANE_BACKEND"] = "postgres"
        overrides["DSX_CONNECT_V2_POSTGRES__URL"] = "postgresql://dsx:dsx@127.0.0.1:5432/dsx_connect_v2"
        overrides["DSX_CONNECT_V2_POSTGRES__AUTO_APPLY_SCHEMA"] = "true"
    if ctx.obj.get("with_rabbit_docker"):
        overrides["DSX_CONNECT_V2__JOB_BUS_BACKEND"] = "rabbitmq"
        overrides["DSX_CONNECT_V2_RABBITMQ__URL"] = "amqp://dsx:dsx@127.0.0.1:5672/%2F"
        overrides["DSX_CONNECT_V2_RABBITMQ__PUBLISHER_CONFIRMS"] = "false"
    if ctx.obj.get("with_dsxa_docker"):
        dsxa_scheme = str(ctx.obj.get("dsxa_scheme", "http"))
        dsxa_host_port = ctx.obj.get("resolved_dsxa_host_port") or ctx.obj.get("dsxa_host_port") or 15000
        overrides["DSX_CONNECT_V2_SCANNER__MODE"] = "dsxa"
        overrides["DSX_CONNECT_V2_SCANNER__BASE_URL"] = f"{dsxa_scheme}://127.0.0.1:{dsxa_host_port}"
        overrides["DSX_CONNECT_V2_SCANNER__VERIFY_TLS"] = str(ctx.obj.get("dsxa_verify_tls", False)).lower()
        auth_token = ctx.obj.get("dsxa_auth_token")
        if auth_token:
            overrides["DSX_CONNECT_V2_SCANNER__DSXA_AUTH_TOKEN"] = str(auth_token)
    else:
        scanner_mode = os.environ.get("DSX_CONNECT_V2_SCANNER__MODE")
        scanner_base_url = os.environ.get("DSX_CONNECT_V2_SCANNER__BASE_URL")
        if scanner_mode is not None:
            overrides["DSX_CONNECT_V2_SCANNER__MODE"] = scanner_mode
        if scanner_base_url is not None:
            overrides["DSX_CONNECT_V2_SCANNER__BASE_URL"] = scanner_base_url
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_PREFETCH_COUNT"] = str(ctx.obj.get("scan_worker_prefetch_count", 1000))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT"] = str(ctx.obj.get("scan_worker_count", 1))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_BATCH_SIZE"] = str(ctx.obj.get("scan_only_completion_batch_size", 1))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_ONLY_COMPLETION_FLUSH_INTERVAL_SECONDS"] = str(ctx.obj.get("scan_only_completion_flush_interval_seconds", 1.0))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_ONLY_RUNTIME_LEASES"] = str(ctx.obj.get("scan_only_runtime_leases", False)).lower()
    overrides["DSX_CONNECT_V2_LOCAL__SCANNER_CLIENT_SCOPE"] = str(ctx.obj.get("scanner_client_scope", "shared"))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_WORKER_SERVICE_IO_THREADED"] = str(ctx.obj.get("scan_worker_service_io_threaded", False)).lower()
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_SIZE"] = str(ctx.obj.get("scan_batch_window_size", 100))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_WAIT_SECONDS"] = str(ctx.obj.get("scan_batch_window_wait_seconds", 0.5))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_BATCH_CONCURRENCY"] = str(ctx.obj.get("scan_batch_concurrency", 6))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ACK_MODE"] = str(ctx.obj.get("scan_batch_ack_mode", "scanned"))
    overrides["DSX_CONNECT_V2_LOCAL__SCAN_BATCH_TRUST_ITEMS"] = str(ctx.obj.get("scan_batch_trust_items", True)).lower()
    overrides["DSX_CONNECT_V2_LOCAL__POLICY_PREFETCH_COUNT"] = str(ctx.obj.get("policy_worker_prefetch_count", 100))
    overrides["DSX_CONNECT_V2_LOCAL__RESULT_SINK_PREFETCH_COUNT"] = str(ctx.obj.get("result_sink_worker_prefetch_count", 1))
    relay_max_active_scan_items = ctx.obj.get("relay_max_active_scan_items")
    if relay_max_active_scan_items is not None:
        overrides["DSX_CONNECT_V2_RELAY__MAX_ACTIVE_SCAN_ITEMS"] = str(relay_max_active_scan_items)
    relay_batch_size = ctx.obj.get("relay_batch_size")
    if relay_batch_size is not None:
        overrides["DSX_CONNECT_V2_RELAY__BATCH_SIZE"] = str(relay_batch_size)
    relay_poll_interval_seconds = ctx.obj.get("relay_poll_interval_seconds")
    if relay_poll_interval_seconds is not None:
        overrides["DSX_CONNECT_V2_RELAY__POLL_INTERVAL_SECONDS"] = str(relay_poll_interval_seconds)
    elif ctx.obj.get("with_postgres_docker"):
        overrides["DSX_CONNECT_V2_RELAY__POLL_INTERVAL_SECONDS"] = str(LOCAL_POSTGRES_RELAY_POLL_INTERVAL_SECONDS)
    return overrides
...
def _resolve_dsxa_image(ctx: typer.Context, env_values: dict[str, str]) -> str:
    return str(ctx.obj.get("dsxa_image") or env_values.get("DSXA_IMAGE") or DSXA_DEFAULT_IMAGE).strip()


def _resolve_dsxa_host_port(ctx: typer.Context, env_values: dict[str, str]) -> int:
    raw_port = ctx.obj.get("dsxa_host_port")
    if raw_port is None:
        raw_port = env_values.get("HOST_PORT") or 15000
    try:
        port = int(raw_port)
    except (TypeError, ValueError) as exc:
        raise RuntimeError(f"dsxa_host_port_invalid:{raw_port}") from exc
    if port < 1:
        raise RuntimeError(f"dsxa_host_port_invalid:{raw_port}")
    return port
...
def _prepare_runtime(
    ...
    *,
    ...
    dsxa_env_values: dict[str, str] | None = None
    if ctx.obj.get("with_dsxa_docker"):
        dsxa_env_values = _dsxa_docker_env(ctx, paths["env"])
        ctx.obj["resolved_dsxa_image"] = _resolve_dsxa_image(ctx, dsxa_env_values)
        ctx.obj["resolved_dsxa_host_port"] = _resolve_dsxa_host_port(ctx, dsxa_env_values)
    ...
    try:
        ...
        if ctx.obj.get("with_dsxa_docker"):
            dsxa_host_port = int(ctx.obj.get("resolved_dsxa_host_port", 15000))
            dsxa_started_by_launcher, message = _ensure_dsxa_container(
                dsxa_container_name,
                image=str(ctx.obj.get("resolved_dsxa_image") or ""),
                host_port=dsxa_host_port,
                container_port=int(ctx.obj.get("dsxa_container_port", 5000)),
                env_values=dsxa_env_values or {},
            )
            print(message)
            _wait_for_dsxa_ready(dsxa_container_name, "127.0.0.1", dsxa_host_port)
            print(
                f"dsxa ready on {ctx.obj.get('dsxa_scheme', 'http')}://"
                f"127.0.0.1:{dsxa_host_port}"
            )
    except Exception:
...
def main(
    ...
    dsxa_image: str = typer.Option("", "--dsxa-image", envvar="DSX_CONNECT_V2_LOCAL__DSXA_IMAGE", help="DSXA Docker image to run; falls back to DSXA_IMAGE from --dsxa-env-file"),
    dsxa_host_port: int | None = typer.Option(None, "--dsxa-host-port", min=1, help="host port for DSXA scanner; falls back to HOST_PORT from --dsxa-env-file, then 15000"),
    ...
    dsxa_env_file: str = typer.Option("", "--dsxa-env-file", help="optional env file containing APPLIANCE_URL, TOKEN, SCANNER_ID, FLAVOR, NO_SSL, AUTH_TOKEN, DSXA_IMAGE, HOST_PORT"),
    dsxa_auth_token: str = typer.Option("", "--dsxa-auth-token", envvar="DSX_CONNECT_V2_LOCAL__DSXA_AUTH_TOKEN", help="optional DSXA REST auth token; also passed to NG scanner config"),
