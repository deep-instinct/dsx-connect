#!/usr/bin/env python3
"""Run Google Cloud Storage connector locally without Docker/K8s."""
...
DEFAULT_STATE_DIR = Path.home() / ".dsx-connect-local" / "google-cloud-storage-connector"
DEFAULT_PORT = 8630

app = typer.Typer(help="Google Cloud Storage connector local runtime manager")
...
def _is_repo_root(path: Path) -> bool:
    return (path / "connectors" / "google_cloud_storage" / "start.py").exists()
...
def _default_env_template(*, state_dir: Path, port: int) -> str:
    _ = state_dir
    return f"""# Google Cloud Storage connector local runtime env (generated)
...
def _service_spec(state_dir: Path, *, port: int, host: str, workers: int, reload: bool) -> ServiceSpec:
    ...
    cmd = [
        python,
        str(repo / "connectors" / "google_cloud_storage" / "start.py"),
        "--host",
        host,
        "--port",
        str(port),
    ...
    return ServiceSpec(
        ...
        command=cmd,
        logfile=paths["logs"] / "google-cloud-storage-connector.log",
        pidfile=paths["run"] / "google-cloud-storage-connector.pid",
        cwd=repo,
    )
