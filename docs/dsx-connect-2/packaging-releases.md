...
3. Commit, tag, push, and watch the DSX-Connect release.
   ...
   ```bash
   git commit -m "Bump DSX-Connect v2 to 2.0.20"
   git tag dsx-connect-v2.0.20
   git push origin main dsx-connect-v2.0.20
...
7. Upgrade the lab only after the release workflows succeed.
   ...
   helm --kube-context k3s-uslab upgrade --install dsx-connect \
     ...
     --version 2.0.20 -n dsx-connect --reuse-values --wait --timeout 5m
