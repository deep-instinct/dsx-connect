#!/usr/bin/env python3
"""Run AWS S3 connector locally without Docker/K8s."""
...
DEFAULT_STATE_DIR = Path.home() / ".dsx-connect-local" / "aws-s3-connector"
DEFAULT_PORT = 8600

app = typer.Typer(help="AWS S3 connector local runtime manager")
...
def _is_repo_root(path: Path) -> bool:
    return (path / "connectors" / "aws_s3" / "start.py").exists()
...
def _default_env_template(*, state_dir: Path, port: int) -> str:
    _ = state_dir
    return f"""# AWS S3 connector local runtime env (generated)
...
# Optional S3 endpoint override (e.g. MinIO)
#DSXCONNECTOR_S3_ENDPOINT_URL=
DSXCONNECTOR_S3_ENDPOINT_VERIFY=true

# AWS credentials (or use ~/.aws credentials/profile)
#AWS_ACCESS_KEY_ID=
...
def _service_spec(state_dir: Path, *, port: int, host: str, workers: int, reload: bool) -> ServiceSpec:
    ...
    cmd = [
        python,
        str(repo / "connectors" / "aws_s3" / "start.py"),
        "--host",
        host,
        "--port",
        str(port),
    ...
    return ServiceSpec(
        ...
        command=cmd,
        logfile=paths["logs"] / "aws-s3-connector.log",
        pidfile=paths["run"] / "aws-s3-connector.pid",
        cwd=repo,
    )
...
def cmd_start(
    ...
    if not env_path.exists():
        ...
        print("run: python connectors/aws_s3/local/aws_s3_local.py init")
...
def cmd_status(ctx: typer.Context) -> None:
    ...
    print(f"{spec.name:22} {state:8} pid={pid if pid else '-'} log={spec.logfile}")
    if not alive:
        raise typer.Exit(code=1)
