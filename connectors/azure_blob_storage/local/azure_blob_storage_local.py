#!/usr/bin/env python3
"""Run Azure Blob Storage connector locally without Docker/K8s."""
...
DEFAULT_STATE_DIR = Path.home() / ".dsx-connect-local" / "azure-blob-storage-connector"
DEFAULT_PORT = 8610

app = typer.Typer(help="Azure Blob Storage connector local runtime manager")
...
def _is_repo_root(path: Path) -> bool:
    return (path / "connectors" / "azure_blob_storage" / "start.py").exists()
...
def _default_env_template(*, state_dir: Path, port: int) -> str:
    _ = state_dir
    return f"""# Azure Blob Storage connector local runtime env (generated)
...
# Azure credentials (pick one method)
...
def _service_spec(state_dir: Path, *, port: int, host: str, workers: int, reload: bool) -> ServiceSpec:
    ...
    cmd = [
        python,
        str(repo / "connectors" / "azure_blob_storage" / "start.py"),
        "--host",
        host,
        "--port",
        str(port),
    ...
    return ServiceSpec(
        ...
        command=cmd,
        logfile=paths["logs"] / "azure-blob-storage-connector.log",
        pidfile=paths["run"] / "azure-blob-storage-connector.pid",
        cwd=repo,
    )
...
def cmd_start(
    ...
    if not env_path.exists():
        ...
        print("run: python connectors/azure_blob_storage/local/azure_blob_storage_local.py init")
...
def cmd_status(ctx: typer.Context) -> None:
    ...
    print(f"{spec.name:28} {state:8} pid={pid if pid else '-'} log={spec.logfile}")
    if not alive:
        raise typer.Exit(code=1)
