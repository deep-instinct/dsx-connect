...
from dsx_connect_v2.local.dsx_connect_v2_local import (
...
def test_default_env_template_mentions_rabbitmq_mode() -> None:
    ...
    assert "DSX_CONNECT_V2__CONTROL_PLANE_BACKEND=postgres" in content
    assert "DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq" in content
    assert "DSX_CONNECT_V2_RABBITMQ__URL=amqp://dsx:dsx@127.0.0.1:5672/%2F" in content
    assert "DSX_CONNECT_V2_POSTGRES__URL=postgresql://dsx:dsx@127.0.0.1:5432/dsx_connect_v2" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_PREFETCH_COUNT=1000" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT=1" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_SIZE=100" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_WAIT_SECONDS=0.5" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_CONCURRENCY=6" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ACK_MODE=scanned" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_TRUST_ITEMS=true" in content
    assert "DSX_CONNECT_V2_LOCAL__SCAN_ONLY_RUNTIME_LEASES=false" in content
    assert "DSX_CONNECT_V2_LOCAL__DSXA_IMAGE=" in content
    assert "APPLIANCE_URL=<your-appliance.deepinstinctweb.com>" in content
    assert "DSX_CONNECT_V2_LOCAL__POLICY_PREFETCH_COUNT=100" in content
    assert "DSX_CONNECT_V2_LOCAL__RESULT_SINK_PREFETCH_COUNT=1" in content
    assert "DSX_CONNECT_V2_RESULT_SINK__BACKEND=stdout" in content
...
def test_runtime_env_overrides_include_relay_tuning_options() -> None:
    ...
    assert overrides["DSX_CONNECT_V2_LOCAL__SCAN_PREFETCH_COUNT"] == "10"
    assert overrides["DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT"] == "3"
    assert overrides["DSX_CONNECT_V2_RELAY__MAX_ACTIVE_SCAN_ITEMS"] == "30"
    assert overrides["DSX_CONNECT_V2_RELAY__BATCH_SIZE"] == "50"
    assert overrides["DSX_CONNECT_V2_RELAY__POLL_INTERVAL_SECONDS"] == "0.25"
...
def test_runtime_env_overrides_configure_local_dsxa_docker() -> None:
    ...
    assert overrides["DSX_CONNECT_V2_SCANNER__MODE"] == "dsxa"
    assert overrides["DSX_CONNECT_V2_SCANNER__BASE_URL"] == "http://127.0.0.1:15000"
    assert overrides["DSX_CONNECT_V2_SCANNER__VERIFY_TLS"] == "false"
    assert overrides["DSX_CONNECT_V2_SCANNER__DSXA_AUTH_TOKEN"] == "rest-token"


def test_runtime_env_overrides_allow_scanner_env_without_local_dsxa(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("DSX_CONNECT_V2_SCANNER__MODE", "stub")
    monkeypatch.setenv("DSX_CONNECT_V2_SCANNER__BASE_URL", "")
    ...
    assert overrides["DSX_CONNECT_V2_SCANNER__MODE"] == "stub"
    assert overrides["DSX_CONNECT_V2_SCANNER__BASE_URL"] == ""
...
def test_runtime_env_overrides_use_resolved_dsxa_host_port() -> None:
    ...
    assert overrides["DSX_CONNECT_V2_SCANNER__BASE_URL"] == "http://127.0.0.1:16000"
...
def test_service_specs_include_api_relay_scan_policy_remediation_delivery_and_dianna(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text("DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq\n")
...
def test_service_specs_can_start_multiple_scan_worker_processes(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text(
        ...
        "DSX_CONNECT_V2_LOCAL__SCAN_PREFETCH_COUNT=4\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT=3\n"
    )
...
def test_service_specs_can_use_per_task_scanner_client_scope(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text(
        "DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq\n"
        "DSX_CONNECT_V2_LOCAL__SCANNER_CLIENT_SCOPE=per-task\n"
    )
...
def test_service_specs_can_thread_scan_worker_service_io(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text(
        "DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_WORKER_SERVICE_IO_THREADED=true\n"
    )
...
def test_service_specs_can_configure_scan_batch_window(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text(
        "DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_SIZE=8\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_WINDOW_WAIT_SECONDS=0.02\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_CONCURRENCY=8\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_ACK_MODE=accepted\n"
...
def test_service_specs_can_disable_scan_batch_trusted_items(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text(
        "DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_BATCH_TRUST_ITEMS=false\n"
    )
...
def test_select_service_specs_selects_all_scan_worker_processes_by_role(tmp_path: Path) -> None:
    (tmp_path / ".env.local").write_text(
        "DSX_CONNECT_V2__JOB_BUS_BACKEND=rabbitmq\n"
        "DSX_CONNECT_V2_LOCAL__SCAN_WORKER_COUNT=2\n"
    )
    specs = _service_specs(tmp_path)
...
def test_ensure_rabbitmq_container_does_not_use_rm(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.local.dsx_connect_v2_local._docker_run", fake_docker_run)
...
def test_ensure_dsxa_container_requires_runtime_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("dsx_connect_v2.local.dsx_connect_v2_local._docker_run", lambda args: Result())
...
def test_wait_for_dsxa_ready_tolerates_registration_conflict_when_app_is_running(
    ...
    monkeypatch.setattr(
        "dsx_connect_v2.local.dsx_connect_v2_local._rabbitmq_container_state",
        lambda container_name: "running",
    )
    monkeypatch.setattr(
        "dsx_connect_v2.local.dsx_connect_v2_local._docker_logs",
...
def test_wait_for_rabbitmq_ready_reports_container_state_and_logs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "dsx_connect_v2.local.dsx_connect_v2_local._docker_exec",
