...
`dsx_connect_v2` is a separate application boundary for the new DSX-Connect architecture.
...
- runtime-agnostic stage message contracts in `dsx_connect_v2/jobs/contracts.py`
...
1. `dsx_connect_v2` may depend on neutral packages such as `shared` or SDKs, but must not import `dsx_connect`.
...
cd dsx_connect_v2
pip install -e .
python -m uvicorn dsx_connect_v2.app:app --reload
```
...
dsx-connect-v2-relay --once
dsx-connect-v2-scan-worker
dsx-connect-v2-policy-worker
dsx-connect-v2-remediation-worker
dsx-connect-v2-result-sink-worker
dsx-connect-v2-delivery-worker
dsx-connect-v2-dianna-worker
dsx-connect-v2-local init
dsx-connect-v2-local foreground
dsx-connect-v2-local debug --service api --service scan-worker
dsx-connect-v2-local --with-rabbit-docker foreground
dsx-connect-v2-local --with-postgres-docker --with-rabbit-docker foreground
```
...
- `dsx-connect-v2-delivery-worker` remains as a compatibility alias
...
- `dsx-connect-v2-local foreground` starts the API, relay, scan worker, policy worker, remediation worker, result-sink worker, and DIANNA worker together for local testing
- `dsx-connect-v2-local debug --service ...` starts only the selected services and does not fail-fast when one child exits, which is more practical for debugger attach workflows
- `dsx-connect-v2-local --with-rabbit-docker foreground` will also start a local `rabbitmq:3-management` container if Docker is available and the named container is not already running
- `dsx-connect-v2-local --with-postgres-docker --with-rabbit-docker foreground` is the intended multi-process stub pipeline mode, because separate API/worker processes need shared PostgreSQL state
...
python -m uvicorn dsx_connect_v2.app:app --host 127.0.0.1 --port 8093
...
- `DSX_CONNECT_V2_SCANNER__MODE=stub|auto|dsxa`
- `stub` keeps the current synthetic scan behavior for local pipeline testing
- `dsxa` uses the real DSXA SDK path and requires:
  - `DSX_CONNECT_V2_SCANNER__BASE_URL`
  - optional `DSX_CONNECT_V2_SCANNER__DSXA_AUTH_TOKEN`
  - optional `DSX_CONNECT_V2_SCANNER__PROTECTED_ENTITY`; set this only to a valid Deep Instinct protected entity id when your scanner deployment requires one. This is not the DSXA scanner `SCANNER_ID`.
  - optional `DSX_CONNECT_V2_SCANNER__VERIFY_TLS`
...
- reader strategy is resolved in this order:
  ...
  - `DSX_CONNECT_V2_READERS__DEFAULT_STRATEGY`
...
```bash
python3 scripts/validate_v2_proxy_reader.py --poll
```
...
```bash
python3 scripts/validate_v2_proxy_reader.py --scan-only --poll
```
...
```bash
python3 scripts/validate_v2_proxy_reader.py --scan-only --sample-kind eicar --poll
```
...
```bash
python3 scripts/validate_v2_batch_proxy_reader.py --scan-only --poll
```
...
```bash
python3 scripts/validate_v2_batch_proxy_reader.py --scan-only --sample-kind eicar --item-count 6 --poll --result-sink-path /tmp/dsx-connect-v2-results.jsonl
```
...
```bash
python3 scripts/validate_v2_batch_proxy_reader.py --sample-kind eicar --item-count 3 --poll --result-sink-path /tmp/dsx-connect-v2-results.jsonl
```
...
```bash
dsx-connect-v2-local --with-postgres-docker --with-rabbit-docker --scan-worker-prefetch-count 2 foreground
python3 scripts/validate_v2_batch_proxy_reader.py --scan-only --sample-kind eicar --item-count 6 --poll --min-concurrent-scans 2
```
...
```bash
export DSX_CONNECT_V2_SCANNER__MODE=dsxa
export DSX_CONNECT_V2_SCANNER__BASE_URL=http://127.0.0.1:15000
```
...
- `DSX_CONNECT_V2__CONTROL_PLANE_BACKEND=auto|memory|postgres`
- `DSX_CONNECT_V2_POSTGRES__URL=postgresql://...`
...
- `DSX_CONNECT_V2__JOB_BUS_BACKEND=memory|rabbitmq|auto`
- `DSX_CONNECT_V2_RELAY__BATCH_SIZE=100`
- `DSX_CONNECT_V2_RELAY__POLL_INTERVAL_SECONDS=5.0`
...
- queue message models in `dsx_connect_v2/jobs/contracts.py` now use typed bodies for scan results, policy decisions, remediation plans/results, DIANNA results, and delivery results
...
- `dsx_connect_v2/workers/scan_worker.py` is a minimal in-process scan worker stub
- `dsx_connect_v2/workers/policy_worker.py` is a minimal in-process policy worker stub
- both operate on typed queue messages and drive the execution API/service through typed stage callbacks

These are modeled in `dsx_connect_v2/jobs/contracts.py` and are intended to remain valid whether the eventual consumers are plain RabbitMQ consumers or Celery workers.
...
- current runtime knobs:
  - `DSX_CONNECT_V2_RABBITMQ__RETRY_MAX_ATTEMPTS`
  - `DSX_CONNECT_V2_RABBITMQ__RETRY_DELAY_MS`
...
The current topology summary lives in `dsx_connect_v2/jobs/topology.py`.
...
- `dsx-connect-v2-relay --once` flushes pending outbox records once and exits
- `dsx-connect-v2-relay` runs a simple polling relay loop
...
```bash
python3 -m pytest dsx_connect_v2/tests -q
```
...
```bash
dsx-connect-v2-relay --batch-size 100 --poll-interval-seconds 5
```

Optional PostgreSQL repository tests:

- set `DSX_CONNECT_V2_TEST_POSTGRES_URL=postgresql://...`
...
- `dsx_connect_v2/api`: FastAPI routes
- `dsx_connect_v2/api/routes/control_plane.py`: machine-oriented API family
- `dsx_connect_v2/api/routes/execution.py`: scan-path execution API family
- `dsx_connect_v2/api/routes/ui.py`: frontend-oriented API family
- `dsx_connect_v2/config.py`: settings and feature gates
- `dsx_connect_v2/control_plane`: control-plane domain and repositories
- `dsx_connect_v2/jobs`: canonical domain-job models and queue topology
- `dsx_connect_v2/integrations`: integration contracts and capability declarations
- `dsx_connect_v2/readers`: worker-side reader abstractions
- `dsx_connect_v2/workers`: worker runtime entrypoints and orchestration
