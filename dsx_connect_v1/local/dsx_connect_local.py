...
def _is_repo_root(path: Path) -> bool:
    return (path / "dsx_connect_v1" / "dsx-connect-api-start.py").exists()
...
def _service_specs(state_dir: Path, worker_pool: str, worker_concurrency: int) -> list[ServiceSpec]:
    ...
    return [
        ...
        ServiceSpec(
            name="api",
            command=[python, str(repo / "dsx_connect_v1" / "dsx-connect-api-start.py")],
        ...
        ServiceSpec(
            name="workers",
            command=[python, str(repo / "dsx_connect_v1" / "dsx-connect-workers-start.py")],
