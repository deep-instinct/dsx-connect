#!/usr/bin/env python3
"""Run Salesforce connector locally without Docker/K8s."""
...
DEFAULT_STATE_DIR = Path.home() / ".dsx-connect-local" / "salesforce-connector"
DEFAULT_PORT = 8670

app = typer.Typer(help="Salesforce connector local runtime manager")
...
def _is_repo_root(path: Path) -> bool:
    return (path / "connectors" / "salesforce" / "start.py").exists()
...
def _default_env_template(*, state_dir: Path, port: int) -> str:
    _ = state_dir
    return f"""# Salesforce connector local runtime env (generated)
...
DSXCONNECTOR_SF_AUTH_METHOD=auto
DSXCONNECTOR_SF_JWT_PRIVATE_KEY=
DSXCONNECTOR_SF_JWT_PRIVATE_KEY_FILE=
...
def _service_spec(state_dir: Path, *, port: int, host: str, workers: int, reload: bool) -> ServiceSpec:
    ...
    cmd = [
        python,
        str(repo / "connectors" / "salesforce" / "start.py"),
        "--host",
        host,
        "--port",
        str(port),
    ...
    return ServiceSpec(
        ...
        command=cmd,
        logfile=paths["logs"] / "salesforce-connector.log",
        pidfile=paths["run"] / "salesforce-connector.pid",
        cwd=repo,
    )
...
def cmd_start(
    ...
    if not env_path.exists():
        ...
        print("run: python connectors/salesforce/local/salesforce_local.py init")
...
def cmd_status(ctx: typer.Context) -> None:
    ...
    print(f"{spec.name:24} {state:8} pid={pid if pid else '-'} log={spec.logfile}")
    if not alive:
        raise typer.Exit(code=1)
